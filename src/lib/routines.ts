/**
 * Pure logic for the routine generator (no React, no I/O).
 *
 * Extracted from `RoutineGenerator` so tests can verify that each
 * parameter (level, goal, days, duration) actually changes the routine.
 * The component remains the only renderer; this function only
 * builds the advisory proposal in memory (no storage, no network).
 */

export type RoutineLevel = 'inicial' | 'intermedio'
export type RoutineGoal = 'fuerza' | 'cardio' | 'movilidad' | 'equilibrio' | 'osea' | 'general'
...
export const WARMUPS: Record<RoutineGoal, { inicial: string[]; intermedio: string[] }> = {
  fuerza: {
...
export const EXERCISES: Record<RoutineGoal, { inicial: RoutineExercise[]; intermedio: RoutineExercise[] }> = {
  fuerza: {
    inicial: [
...
export const DAY_FOCUS: Record<RoutineGoal, string[]> = {
...
export const ROUTINE_PROGRESSION: Record<RoutineLevel, string[]> = {
  inicial: [
...
export function exerciseCountFor(minutes: number, level: RoutineLevel): number {
  const base = minutes <= 15 ? 3 : minutes <= 30 ? 4 : minutes <= 45 ? 5 : 6
  return level === 'intermedio' ? base + 1 : base
}

/**
 * Builds the advisory routine for a parameter combination.
 * Same logic the component used: rotates the starting point per day
 * so each day combines different exercises.
 */
export function buildRoutine(
  ...
  const dayPlans: RoutineDayPlan[] = Array.from({ length: days }, (_, i) => {
  ...
  return {
    ...
    days: dayPlans,
    cooldown: ROUTINE_COOLDOWN,
    progression: ROUTINE_PROGRESSION[level],
  }
}
